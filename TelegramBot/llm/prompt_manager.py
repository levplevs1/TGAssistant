...
def process_user_request(user_text, document=None, comment=None):
    ...
    data = create_prompt_data(
        ...
        user_instructions=(
            ...
            "Дай ответ строго по теме. Если запрос пользователя не имеет отношения к госуслугам, прямо укажи на это, но предложи обратиться в соответствующие органы или на портал госуслуг. "
