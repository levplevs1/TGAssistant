...
def create_prompt_data(system_instructions, user_instructions, comment=None, response_format=None, max_tokens=512, temperature=0.5):
    """
...
def process_user_request(user_text, document=None, comment_text=''):
...
def classify_query_with_llm(query: str, comment_text=''):
    ...
    data = create_prompt_data(
        system_instructions=
            ...
            "1. Если запрос содержит передачу показаний счётчиков (например, 'горячая вода 123', 'электричество 55') "
...
def get_meter_readings_llm(query: str):
    data = create_prompt_data(
        system_instructions=
            "Ты — профессиональный классификатор и обработчик показаний счётчиков. Твоя задача — чётко анализировать запросы пользователя, "
            "корректно классифицировать данные и возвращать информативные ответы.\n\n"
            "1. Разделяй запросы на две части:\n"
            "   - Передача показаний счётчиков: показания для газа, воды, электричества или отопления.\n"
            "   - Действия: указания на оплату или выполнение конкретной задачи ('оплати воду', 'оформи показания').\n\n"
            "2. Обрабатывай показания счётчиков:\n"
            "   - Категории: 'вода', 'электричество', 'отопление', 'газ'.\n"
            "   - Сохраняй данные для каждой категории в соответствующие поля.\n"
            "   - Если данные относятся к нескольким категориям, сохраняй их все.\n\n"
            "3. Определяй действия:\n"
            "   - Если запрос содержит действие ('оплати', 'оформи'), классифицируй как 'услуга' и добавляй это в 'message'.\n"
            "   - Действие должно быть связано с одной категорией. Например, 'оплати воду' указывает на 'вода'.\n\n"
            "4. Проверяй корректность данных:\n"
            "   - Значения должны быть числами (или словами, обозначающими числа).\n"
            "   - Отрицательные значения и некорректные форматы вызывают ошибку ('error: True') с пояснением в 'message'.\n\n"
            "5. Формируй результат в формате JSON:\n"
            "{\n"
            ...
            "}\n\n"
            "6. Для сложных запросов (например, передача показаний и действия):\n"
            "   - Определи ключевое действие (например, 'оплати воду').\n"
            "   - Сохрани показания всех категорий.\n"
            "   - Добавь пояснение в 'message', связанное с действием (например, 'Оплата воды, переданы показания для других категорий').",
    user_instructions=
