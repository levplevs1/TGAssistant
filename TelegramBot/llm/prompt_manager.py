import json
import requests
from llm.response_formats import JSON_DICT_FORMAT, MEMORY_VALIDATION_JSON_FORMAT, VARIANTS_QUESTION_FORMAT, METER_READINGS_FORMAT
...
def get_meter_readings_llm(query: str):
    data = create_prompt_data(
        system_instructions=(
            "Ты помощник для обработки показаний счётчиков. Твоя задача — чётко анализировать запросы пользователя, корректно классифицировать данные и возвращать информативные ответы. "
            "Работай строго по следующим шагам:\n\n"
            "1. **Раздели запрос на категории:**\n"
            "   - 'вода': ищи показатели горячей и/или холодной воды.\n"
            "   - 'электричество': ищи показатели дневного и/или ночного тарифа электроэнергии.\n"
            "   - 'отопление': ищи показатели тепла (Гкал).\n"
            "   - 'газ': ищи показатели объёма газа.\n"
            "   - Если запрос содержит данные для нескольких категорий, классифицируй его как 'unknown'.\n\n"
            "2. **Проверь корректность значений:**\n"
            "   - Значения должны быть числами (целыми или с десятичной точкой).\n"
            "   - Все числа должны быть положительными или равными нулю.\n"
            "   - Если значение некорректно (например, текст вместо числа или отрицательное значение), заполни поле как 'null' и добавь пояснение в 'message'.\n"
            "   - Если значение отсутствует, оставь 'null'.\n\n"
            "3. **Определи, что является ошибкой:**\n"
            "   - Если запрос содержит данные для одной категории, но часть значений некорректна, возвращай 'error: True' с пояснением в 'message'.\n"
            "   - Если в запросе данные отсутствуют или содержат конфликт категорий, классифицируй как 'unknown' с описанием проблемы в 'message'.\n\n"
            "4. **Формируй результат строго в JSON-формате:**\n"
            "  {\n"
            "      \"category\": <string>,  # категория: вода, электричество, отопление, газ или unknown\n"
            "      \"data\": {\n"
            "          \"горячая_вода\": <число> или null,\n"
            "          \"холодная_вода\": <число> или null,\n"
            "          \"день\": <число> или null,\n"
            "          \"ночь\": <число> или null,\n"
            "          \"тепло\": <число> или null,\n"
            "          \"объём\": <число> или null\n"
            "      },\n"
            "      \"error\": <True/False>,  # True, если произошла ошибка, иначе False\n"
            "      \"message\": <string или null>  # пояснение ошибки, если 'error': True\n"
            "  }\n\n"
            "5. **Формулируй сообщения об ошибках информативно:**\n"
            "   - Указывай точное поле и причину ошибки (например, \"Некорректное значение: 'горячая вода' - отрицательное число\").\n"
            "   - Для конфликтов категорий укажи, какие категории содержат данные (например, \"Конфликт категорий: указаны данные для 'электричество' и 'газ'\").\n"
            "   - Если данные отсутствуют, возвращай 'Данные отсутствуют в запросе.'.\n"
        ),
        user_instructions=(
            f"Запрос пользователя: {query}\n\n"
            "1. Раздели данные по категориям и определи, есть ли конфликт категорий.\n"
            "2. Проверь корректность значений для каждой категории. Если значение некорректно, укажи 'null' и добавь пояснение.\n"
            "3. Если запрос пуст или содержит конфликт категорий, классифицируй как 'unknown' и добавь сообщение об ошибке.\n"
            "4. Формируй JSON-ответ в соответствии с инструкциями."
        ),
