...
## Шаг 3: Увеличьте контекст модели
Для работы вашего проекта с длинными запросами или историями необходимо увеличить длину контекста модели до **16k токенов** или больше.

### Как изменить контекст модели:
1. Перейдите во вкладку **My Models** (Мои модели).
2. Найдите модель `Qwen2.5 14B Instruct` в списке скачанных моделей.
3. Нажмите на значок **шестерёнки** рядом с названием модели.
4. В появившихся настройках найдите параметр **Context Length** (Длина контекста).
5. Используя ползунок, установите значение на **16k**.
